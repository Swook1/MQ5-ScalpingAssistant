...
Magic number is auto-derived from `_Symbol` **only** so two iScalp instances on different pairs never interfere, while the same pair keeps one identity across timeframes.

Deterministic formula:

```
magic = 79000000 + CRC16(_Symbol)
```
...
Timeframe is deliberately **excluded**: magic keys the persisted panel state (`iSc_{magic}_*` GlobalVariables) and the chart-line object names, so including `_Period` would orphan orders, wipe lines and reset the panel on every TF switch.
